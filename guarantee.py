...
def guarantee_content(date, customer_name, customer_id, amount):
    ...
    transfer_word_to_pdf(folder_path,customer_name,word_file_path)

def transfer_word_to_pdf(folder_path,customer_name,word_file_path):
    ...
    if len(customer_id) != 8:
        while True:
            ...
            if len(customer_id) == 8:
                break
    amount = input('保證金金額: ')
    guarantee_content(date, customer_name, customer_id, amount)